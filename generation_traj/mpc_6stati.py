...
def mpc_step(
    x0,
    u_prev,     # input applicato al passo precedente, serve per i vincoli su delta-u
    path_ref,      # array shape (N+1, 3) con riferimento di posizione e heading : [X*, Y*, phi*] per k=0..N
    Ts=0.02,   # passo di campionamento 
    N=20,      # orizzonte di predizione
    params=None,
    # Pesi costo
    q_c=6.0,       # peso errore laterale rispetto al path 
    q_phi=0.5,     # peso  errore di heading 
    q_vx=0.5,      # peso tracking  velocità longitudinale vx 
    R=np.diag([0.02, 2.0]),        # penalità quadratica sugli input
    Rd=np.diag([0.01, 5.0]),       # penalità quadratica sulle variazioni di input delta-u
    vref=None,     # float o array shape (N+1,)
    # Vincoli su input e rate
    u_bounds=(( -1.0,  1.0),   #  box sui comandi d
              ( -0.6,  0.6)),  # delta [rad]
    du_bounds=((-0.5, 0.5),    # box sulle differenze tra comandi successivi Δd per step
    ...
    if vref is None:
        vref = np.full(N + 1, x0[3])  # mantiene vx iniziale costante su tutto l'orizzonte
    elif np.isscalar(vref):
        vref = np.full(N + 1, float(vref)) #lo estende a (N+1,)
    else:
        vref = np.asarray(vref).reshape(N + 1) # lo usa tal quale (shape N+1)

#riferimento di posizione / orientamento viene da path_ref, mentre il riferimento di velocità longitudinale è vref

    # 1) Traiettoria di riferimento nominale (x̄, ū) con Eulero
    xbar = np.zeros((6, N + 1)) # traiettoria nominale degli stati
    ubar = np.zeros((2, N)) # traiettoria nominale degli input
    xbar[:, 0] = x0
    for k in range(N):
        ubar[:, k] = u_pr # input costante (ultimo noto) su tutto l'orizzonte
        xbar[:, k + 1] = xbar[:, k] + Ts * f_cont(xbar[:, k], ubar[:, k], p)

    # lo scopo è avere un punto di linearizzazione per ogni step k e lo fa propagando lo stato con dinamica non lineare f_cont usando Eulero in avanti 
 
    # 2) Precalcolo Ad, Bd, g per k=0..N-1

    # per ogni k, calcolo le matrici della dinamica affine e discreta 
    ...
    # il primo stato è vincolato allo stato reale iniziale x0
    X = cp.Variable((6, N + 1))
    U = cp.Variable((2, N))
    ...
    for k in range(N):
        ...
        constraints += [X[:, k + 1] == Ad @ X[:, k] + Bd @ U[:, k] + g]  #impone la dinamica lineare e affine calcolata al punto nominale 
    ...
    for k in range(N):
        ...
        obj += q_phi * cp.square(X[2, k] - Pr[k]) # phi - phi_ref

        # tracking vx
        obj += q_vx * cp.square(X[3, k] - vref[k]) # vx - vx_ref

        # costo input
        obj += cp.quad_form(U[:, k], R) # u'Ru  R è una matrice per pesare separatamente d e delta
        ...
        if k == 0:
            ...
            du = U[:, k] - U[:, k-1]
        obj += cp.quad_form(du, Rd) # du'Rdu  Rd è una matrice per pesare separatamente Δd e Δdelta ed evitare comandi a "scatti"
    ...
    try:
        ...
        return u_pr, f"error: {type(e).__name__}: {e}", {} # in caso di eccezione, ritorna u_prev e un messaggio di errore 

    if status not in ("optimal", "optimal_inaccurate"):
        return u_pr, status, {} # se lo stato non è optimal o optimal_inaccurate, ritorna u_prev
    


    u_cmd = np.array([U[0, 0].value, U[1, 0].value]).reshape(2) # se ottimale, estrae il primo comando della sequenza ottima
    # questo è l'input da applicare *adesso* , lo schema receding horizon applica solo il primo comado poi si ripete tutto al passo successivo con misure aggiornate
    info = {
        "status": status, #stato del solver
        "objective": prob.value, #valore del costo ottimo
        "X": X.value, #traiettoria di stato ottima (6,(N+1))
        "U": U.value, #sequenza di input ottima (2,N)
        "path_ref": path_ref, #riferimento di posizione e heading
        "vref": vref #riferimento di velocità longitudinale
    }
